...
// LIMIT
$tampilDaftarPembelian = $globalicClass->tampilDaftarPembelian("pembelian.*,buku.*,u1.username AS uPeminjam","INNER JOIN user AS u1 ON pembelian.id_peminjam=u1.id_user WHERE pembelian.id_peminjam='$id' ORDER BY id_pembelian DESC LIMIT $start,$limit");

// COUNT
$tampilDaftarPembelian1 = mysqli_query($globalicClass->koneksi,"SELECT COUNT(id_pembelian) AS id FROM pembelian WHERE pembelian.id_peminjam='$id'");
...
function hiddenBatal($status,$idBeli){
  switch ($status) {
    // Jika status belum bayar, menampilkan tombol modal batal pembelian + mengisi alasan
    case 'Belum Bayar':
    return "<button type='button' class='btn btn-danger me-1' data-bs-toggle='modal' data-bs-target='#batalModal$idBeli'>Batal</button>";
    break;
  ...
  switch ($status) {
    ...
    if ($metBayar !== 'Cash') {
      if (isset($cek) && $cek > 0) {
        ...
        return "
        <form action='buktibayar.php' method='POST' class='d-inline'>
        <input type='number' name='id_pembelian' value='$idBeli' hidden>
        <input type='submit' class='btn btn-success' value='Unggah Bukti'>
        </form>";
      }
    }else{
      return '';
...
<head>
  ...
  <title>Daftar Pembelian - Peminjam</title>
</head>
<body id="home">
          ...
          <tbody class="table table-bordered border-secondary table-light">
            ...
            foreach($tampilDaftarPembelian as $d){
              ...
              <tr>
                ...
                <td>
                  <div class="d-inline">
                    <?= hiddenBatal($d['status'],$d['id_pembelian']).hiddenLihatStruk($d['status'],$d['id_pembelian']).hiddenBukti($d['status'],$d['id_pembelian'],$d['metode_pembayaran'])?>
                  </div>
                </td>
              </tr>
              <!-- Modal Batal -->
                  ...
                  <div class="modal-content">
                    <form action="../proses.php" method="POST">
                      ...
                      <div class="modal-footer">
                        ...
                        <button type="submit" name="btn" value="BatalBeli" class="btn btn-danger">Batal</button>
                      </div>
                    </form>
                  </div>
                </div>
              ...
              <div class="modal fade" id="modalBukti<?= $d['id_pembelian']?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                  <div class="modal-content modal-fullscreen-sm-down">
                    ...
                    <div class="modal-body">
                      <img src="bukti/<?= $d['bukti_pembayaran']?>" alt="Bukti Bayar" class="img-fluid d-flex mx-auto" style="width: 40%">
                      <h5 class="mt-2">Tunggu status berubah menjadi 'Lunas' dari Petugas</h5>
                    </div>
                    <div class="modal-footer">
                      <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Tutup</button>
                    </div>
